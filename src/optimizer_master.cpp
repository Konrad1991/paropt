...
typedef double (*fctptr2)(std::vector<double> &param_combi_start, OS2 ode_system, time_state_information_a2a &solv_param_struc);
//std::mutex mtx;

void helper_fct (arma::mat inp, arma::vec & errors, int index, int number_of_rows, const int nvals, OS2 odes, time_state_information_a2a model,
                 fctptr2 fctptr){
/*
  assert(nvals > 0);
  ...
  OS2 ODE = odes;
  time_state_information_a2a MODEL = model;
  mtx.unlock();
...
Optimizer_a2a::Optimizer_a2a(
        std::vector<double> t_lb,
        std::vector<double> t_ub,
        settingsPSO_a2a t_set_pso,
        time_state_information_a2a t_model,
        double (*fctptr2)(std::vector<double> &param_combi_start, OS2 ode_system, time_state_information_a2a &solv_param_struc) ,
        OS2 t_odes ) { //Rcpp::XPtr<OS2> t_odes
...
void Optimizer_a2a::get_best_particle_param_values(std::vector<double> & temp) {
...
double Optimizer_a2a::pso() { // (labled with ! need check)
