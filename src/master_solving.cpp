...
Rcpp::List master_solving(
  ...
  OS2 ode_system = *fctptr;

  Rcpp::NumericMatrix DF(integration_times.size(),init_state.size());
  if(solvertype == "bdf") {
    smsq = solver_bdf_save_a2a(param_combi_start, ode_system, param_model, DF);
  }
  else if(solvertype == "ADAMS") {
    smsq = solver_adams_save_a2a(param_combi_start, ode_system, param_model, DF);
  } else if(solvertype == "ERK") {
    smsq = solver_erk_save_a2a(param_combi_start, ode_system, param_model, DF);
  } else if(solvertype == "ARK") {
    smsq = solver_ark_save_a2a(param_combi_start, ode_system, param_model, DF);
  } else {
